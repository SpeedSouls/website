...
import { ref, inject, nextTick, watch, computed, useAttrs } from 'vue'
...
const renderedPrefix = computed<string>(() =>
	render(props.prefix || '', values.value)
)

const renderedSuffix = computed<string>(() =>
	render(props.suffix || '', values.value)
)
...
function transform(value: string) {
	return value
		.split('/')
		.map((segment) =>
			slugify(segment, { separator: '-', preserveTrailingDash: true }).slice(
				0,
				props.length
			)
		)
